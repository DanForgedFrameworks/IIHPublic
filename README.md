# IIH UK Resource Library

Patient information leaflets, clinical resources, and digital assets produced by **IIH UK** for people living with Idiopathic Intracranial Hypertension, their families and carers, and the healthcare professionals who support them.

**IIH UK** is a registered charity in England & Wales (1143522) and Scotland (SC043294).  
Website: [www.iih.org.uk](https://www.iih.org.uk)

---

## What is IIH?

Idiopathic Intracranial Hypertension (IIH) is a rare neurological condition caused by raised pressure around the brain and spinal cord. It predominantly affects women of childbearing age and, if untreated, can cause permanent sight loss. IIH UK supports patients, families, carers, and clinicians with evidence-based information and resources.

---

## Resource Categories

### Patient and Family Resources
Information for people living with IIH, their families, friends, and carers. Written in plain English for general readability.

Available in `01 Patient and Family/`:

| Reference | Title | Version |
|---|---|---|
| IIHPT001 | Acetazolamide Drug Information | v3.0 |
| IIHPT002 | Disability Benefits Guide | v1.0 |
| IIHPT003 | Headaches in IIH | v3.0 |
| IIHPT004 | Information for Family, Friends and Carers | v3.2 |
| IIHPT004 | Information for Family, Friends and Carers (Digital — Cards &amp; List) | v3.3 HTML |
| IIHPT005 | IIHWOP Patient Information Booklet | v4.0 |
| IIHPT006 | What is IIH Patient Information Booklet | v4.0 |
| IIHPT007 | IIH and Weight Management | v5.0 |
| IIHPT008 | Medication Overuse Headache in IIH | v3.0 |
| IIHPT009 | Shunts and IIH | v5.0 |
| IIHPT010 | Venous Sinus Stenting for IIH | v4.0 |
| IIHPT011 | Optic Nerve Sheath Fenestration for IIH | v3.0 |
| IIHPT012 | Intracranial Pressure Monitoring | v2.0 |
| IIHPT013 | IIH and Education | v3.2 |
| IIHPT014 | Employment with IIH | v1.0 |
| IIHPT015 | Visual Problems in IIH | v3.0 |
| IIHPT016 | When I Go to the Eye Clinic | v2.0 |

### Clinical and Healthcare Professional Resources
Resources for orthoptists, neuro-ophthalmologists, neurologists, and allied health clinicians.

Available in `02 Clinical HCP/`:

| Reference | Title | Version |
|---|---|---|
| IIHHCP001 | BIOS Orthoptic Led IIH Service Presentation | v1.0 |

### Reference and Research
Open access academic papers and guidelines for researchers, policy makers, and clinicians.

Available in `03 Reference and Research/`:

| Reference | Title | Version |
|---|---|---|
| IIHREF001 | 2018 Consensus Guidelines IIH (Mollan et al. JNNP 2018) | v1.0 |
| IIHREF002 | 2018 JLA Research Priorities IIH (Mollan et al. BMJ Open 2019) | v1.0 |
| IIHREF003 | 2026 RISE IIH Standards Call (MacKeith et al. Eye 2026) | v1.0 |
| IIHREF004 | 2018 JLA PSP Contributing Uncertainties | v1.0 |

---

## Accessibility

All digital resources in this library are produced to meet:
- **WCAG 2.2 Level AA** (Web Content Accessibility Guidelines)
- **EN 301 549** (European accessibility standard for ICT products)

Interactive HTML assets are fully self-contained single files with no external dependencies — they work offline and on any device.

---

## Licence

Patient and clinical resources produced by IIH UK are published under  
**Creative Commons Attribution — Non-Commercial 4.0 International (CC BY-NC 4.0)**.

You are free to share and adapt these materials for non-commercial purposes, provided you give appropriate credit to IIH UK and indicate if changes were made.

Academic reference papers in `03 Reference and Research/` retain their original open access licences (CC BY 4.0 — see individual documents).

---

## Contributing

This repository is maintained by IIH UK volunteers. If you have found an error or wish to contribute, please contact IIH UK via [www.iih.org.uk](https://www.iih.org.uk).

---

*IIH UK — Registered Charity England &amp; Wales 1143522 | Scotland SC043294*
